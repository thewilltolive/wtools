...
static void test_bkv_u64(void){
    ...
    uint8_t     *l_ptr=NULL;
    int          l_ptrlen;
#define U64_TEST_KEY (878)
    int16_t      l_key=U64_TEST_KEY;
    uint64_t     l_value64=9038720987703;
    uint16_t     l_keys[2] = { U64_TEST_KEY, BKV_KEY_INVALID };
    ...
    CU_ASSERT_EQUAL_FATAL(BKV_OK,bkv_create(&l_bkv_create,&l_handle));
    CU_ASSERT_EQUAL_FATAL(BKV_OK,bkv_kv_map_open(l_handle,BKV_NO_KEY));
    CU_ASSERT_EQUAL_FATAL(BKV_OK,bkv_kv_u64_add(l_handle,l_key,l_value64));
    CU_ASSERT_EQUAL_FATAL(BKV_OK,bkv_kv_map_close(l_handle));
    CU_ASSERT_EQUAL_FATAL(BKV_OK,bkv_get_head(l_handle,&l_ptr,&l_ptrlen));
    CU_ASSERT_EQUAL_FATAL(BKV_OK,bkv_val_init(&l_val,l_ptr));
    CU_ASSERT_EQUAL_FATAL(BKV_OK,bkv_val_get2(&l_val,&l_keys[0],1,&l_out_value));
    CU_ASSERT_EQUAL_FATAL(l_out_value.u.number.int64,l_value64);
    bkv_destroy(l_handle);
    bkv_term();
}
...
int main(int argc, char **argv)
{
    ...
    if (NULL == CU_add_test(pSuite, "test_bkv_u64", test_bkv_u64)) {
        return CU_get_error();
    }
