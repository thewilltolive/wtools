...
bkv_to_json_float(void *p_data, uint8_t *p_ptr, bkv_key_t key, float f){
    ...
    if (NULL != l_ctx){
        if (BKV_ARRAY_KEY == key){
            ...
            if (BKV_OK != bkv_val_get2(&l_ctx->dico_val,&l_keys[0],1,&l_val)){
                lg_print(BKV_ERROR_ST_ERROR,__FUNCTION__,__LINE__,
...
static bkv_parse_retval_t
bkv_to_json_boolean(void *p_data, uint8_t *p_ptr, bkv_key_t key, bool v){
    bkv_to_json_ctx_t *l_ctx=(bkv_to_json_ctx_t*)p_data;
    bkv_key_t          l_keys[2]= { key, BKV_KEY_INVALID};
    int                l_ret=0;
    bkv_val_t          l_val;
    ...
    if (NULL != l_ctx){
        if (BKV_ARRAY_KEY == key){
            if (BKV_OK != l_ctx->parser->to_json_array_boolean_fn(l_ctx,
                                                                v)){
                l_ret|=BKV_PARSE_ACTION_STOP_LOOP;
            }
        }
            ...
            else if (BKV_OK != l_ctx->parser->to_json_boolean_fn(l_ctx,
                                                               l_val.u.string.str,
                                                               l_val.u.string.len,
                                                               v)){
                l_ret|=BKV_PARSE_ACTION_STOP_LOOP;
